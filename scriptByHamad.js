...
<tbody>       
 ${Object.keys(student.marks).map(subject => `<tr> <td class="auto-style1"
                                style="border: 1px solid green; background-color: darkseagreen; font-weight: bold;">${subject}</td> <td  style="border: 1px solid green; background-color: snow; font-weight: bold; ">&nbsp;<span id="MainContent_Rslt_Table_by_Hamad_${subject}">${student.marks[subject]}</span></td> </tr>`).join('')}
</tbody>
`;
...
Re-appearing</td>${Object.keys(student.marks_subject_re).map(subject => `<td colspan="5" style="border: 1px solid green; background-color: snow;" ><span id="MainContent_subreapps" style="font-weight:bold;" >${student.marks_subject_re[subject]}</span></td>`).join('')}
  ...
  `;
        ...
        const resultHtml_note_hamad = `<td colspan="7" style="border: 1px solid green ;"><b> Note: This is NOT an Official Detailed Marks Certificate <br> Students can apply for Rechecking/Review within ten (10) days of declaration of the result, on the prescribed form, available at www.uhs.edu.pk (Downloads)</b></td>
...
//                to deit notes from json file:  enable it and put json elemetnt to json file that is below this code  named note data for json
//         const resultHtml_note_hamad = `
// ${Object.keys(student.marks_note).map(subject => `<td colspan="7" style="border: 1px solid green ;"><b> Note: This is NOT an Official Detailed Marks Certificate <br> 
//       ${student.marks_note[subject]}</b></td>`).join('')}
...
// ,
//     "marks_note": {
//       "Roll No.hamad": "put this in json so edit note from ther "
//     }
